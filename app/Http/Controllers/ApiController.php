...
//            )->where('created_by', $user->id)->get()->toArray();
    ...
    public function filteruser(Request $request)
    {
        if (isset($request->id)) {
            ...
            if (!empty($id)) {
                $data = User::find($id);
                return response()->json(['FilterUser' => $data]);
            } else {
                return response()->json(['error' => 'user id is required'], 404);
            }
        } else {
            return response()->json(['error' => 'empty request is not allowed'], 401);
        }
    }

    public function gettaskstages(Request $request)
    {
        $data = TaskStage::all();
        return response()->json(['TaskStageData' => $data]);
    }

  
    public function filtertaskstages(Request $request)
    {
        if (isset($request->name) && isset($request->created_by)) {
            $name = $request->name;
            $created_by = $request->created_by;
            if (!empty($name) && !empty($created_by)) {
                $data = TaskStage::where('name', $name)->where('created_by', $created_by)->get();
                return response()->json(['FilterUser' => $data]);
            } else {
                return response()->json(['error' => 'name and created_by is required'], 404);
            }
        } else {
            return response()->json(['error' => 'empty data is not allowed'], 401);
        }
    }
}
