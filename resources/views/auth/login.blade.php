...
@section('content')
    ...
    <div class="">
        <div class="form-group mb-3 input-container">
            <input class="form-control @error('email') is-invalid @enderror" placeholder="Enter Your Email" id=" email"
        ...
        <div class="d-grid">
            <button type="submit" class="login-do-btn btn btn-primary btn-block btn-white-c f-w-600 mt-2"
                    id="login_button" style="color: black; background-color: white;">{{__('Login')}}</button>
        </div>
        @if($settings['enable_signup'] == 'on')
            <p class="my-4 text-center">{{__("Don't have an account?")}} <a
                        href="{{ route('register',!empty(\Auth::user()->lang)?\Auth::user()->lang:'en') }}"
